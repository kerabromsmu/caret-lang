...
enum DiagnosticCatalog {
    ...
    INVALID_COLLECTION_KEY("RUNTIME-INVALID-COLLECTION-KEY", Diagnostic.Phase.RUNTIME,
            ...
            "(?:Collection access key cannot be missing|Collection access key must support equality, got: .*|Sequential Collection key must be an integer, got: .*|Dictionary access key must be a String, got: .*|Key does not satisfy the Collection access contract)", PUBLIC),
    ...
    RUNTIME_MIXED_COLLECTION_SHAPE("RUNTIME-MIXED-COLLECTION-SHAPE", Diagnostic.Phase.RUNTIME,
            ...
            "(?:A collection cannot mix keyed and keyless elements|Collection elements do not satisfy the selected keyed shape|Keyed fields do not satisfy the selected keyless shape|A Set cannot contain associated field values)", PUBLIC),
