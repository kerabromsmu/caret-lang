...
enum DiagnosticCatalog {
    ...
    INVALID_COLLECTION_CALLBACK("RUNTIME-INVALID-COLLECTION-CALLBACK", Diagnostic.Phase.RUNTIME,
            Diagnostic.Codes.INVALID_COLLECTION_CALLBACK,
            "(filter|fold|any|all) (predicate|combine) must be a callable requiring exactly (one argument|two arguments)", PUBLIC),
    INVALID_PREDICATE_RESULT("RUNTIME-INVALID-PREDICATE-RESULT", Diagnostic.Phase.RUNTIME,
            Diagnostic.Codes.INVALID_PREDICATE_RESULT,
            "(filter|any|all) predicate must return Boolean, null, or missing", PUBLIC),
    ...
    INVALID_DICTIONARY_KEY("RUNTIME-INVALID-DICTIONARY-KEY", Diagnostic.Phase.RUNTIME,
            Diagnostic.Codes.INVALID_DICTIONARY_KEY,
            "(?:Dictionary key must be a string, got: .*|Dictionary keys must have one homogeneous sortable type|A Set cannot contain missing)", PUBLIC),
    INVALID_COLLECTION_KEY("RUNTIME-INVALID-COLLECTION-KEY", Diagnostic.Phase.RUNTIME,
            Diagnostic.Codes.INVALID_COLLECTION_KEY,
            "(?:Collection access key cannot be missing|Collection access key must support equality, got: .*|Sequential Collection key must be an integer, got: .*|Dictionary access key must be a String, got: .*|Key does not satisfy the Collection access contract|Transformed Collection key must support equality|A transformed Set member must support equality and cannot be missing|zipWithKeys key cannot be missing|zipWithKeys key must support equality)", PUBLIC),
    ZIP_LENGTH_MISMATCH("RUNTIME-ZIP-LENGTH-MISMATCH", Diagnostic.Phase.RUNTIME,
            Diagnostic.Codes.ZIP_LENGTH_MISMATCH, "zip inputs must have equal lengths", PUBLIC),
    AMBIGUOUS_COLLECTION_SHAPE("RUNTIME-AMBIGUOUS-COLLECTION-SHAPE", Diagnostic.Phase.RUNTIME,
            Diagnostic.Codes.AMBIGUOUS_COLLECTION_SHAPE,
            "Fields without values require a Set or Dictionary contract", PUBLIC),
    ...
    RUNTIME_MIXED_COLLECTION_SHAPE("RUNTIME-MIXED-COLLECTION-SHAPE", Diagnostic.Phase.RUNTIME,
            ...
            "(?:A collection cannot mix keyed and keyless elements|Collection elements do not satisfy the selected keyed shape|Keyed fields do not satisfy the selected keyless shape|A Set cannot contain associated field values|A transformed Collection cannot mix keyed and keyless elements)", PUBLIC),
